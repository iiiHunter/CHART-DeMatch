...
def extract_tick_point_pairs(js):
    ...
    if "x-axis-2" in axes.keys():
        tpp_x_2 = [get_coords(tpp) for tpp in axes['x-axis-2']]
        tpp_x_2 = {ID: coords for ID, coords in tpp_x_2 if ID is not None}
        tpp_x.update(tpp_x_2)
    if "y-axis-2" in axes.keys():
        tpp_y_2 = [get_coords(tpp) for tpp in axes['y-axis-2']]
        tpp_y_2 = {ID: coords for ID, coords in tpp_y_2 if ID is not None}
        tpp_y.update(tpp_y_2)
    return tpp_x, tpp_y
...
def eval_task4(gt_folder, result_folder, img_folder):
    ...
    error_f = open("fail.txt", "w")

    for gt_file in gt_files:
        # if gt_file!='31875.json':
        #     continue
        ...
        if (recall_x + recall_y) / 2. < 1. or (precision_x + precision_y) / 2 < 1.:
            print(recall_x, recall_y, precision_x, precision_y)
            print("filename: ", gt_file)
            error_f.write("%.3f" % ((recall_x + recall_y) / 2) + " " + "%.3f" % (
                        (precision_x + precision_y) / 2) + " " + gt_file + "\n")
